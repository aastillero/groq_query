...
class GroqChat {
  ...
  Future<(GroqResponse, GroqUsage)> sendMessage(
    ...
    GroqMessage? systemRequest;
    if(systemPrompt != null) {
      systemRequest = GroqMessage(content: systemPrompt, role: GroqMessageRole.system, username: username);
    }
  ...
  // sends message queries with vision capability
  Future<(GroqResponse, GroqUsage)> sendMessageWithVision(
      String prompt,
      String img, {
        GroqMessageRole role = GroqMessageRole.user,
        String? username,
    ...
    final content = GroqContent(type: "text", text: prompt);
    //String _cont = "[{'type': 'text', 'text': 'describe the image'}, {'type': 'image_url', 'image_url': {'url': 'https://upload.wikimedia.org/wikipedia/commons/f/f2/LPU-v1-die.jpg'}}]";
    GroqMessage? systemRequest;
    if(systemPrompt != null) {
      systemRequest = GroqMessage(content: systemPrompt, role: GroqMessageRole.system, username: username);
    }
